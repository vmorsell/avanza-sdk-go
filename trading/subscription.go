...
func (s *OrdersSubscription) trySendError(err error) {
	select {
	case s.errors <- err:
	case <-s.ctx.Done():
	}
...
func (s *OrdersSubscription) start() {
	s.wg.Add(1)
	defer s.wg.Done()
...
func (s *OrdersSubscription) connectAndStream() (bool, error) {
...
func (s *OrdersSubscription) setSSEHeaders(req *http.Request) {
	...
	req.Header.Set("Referer", "https://www.avanza.se/min-ekonomi/ordrar.html")
...
func (s *OrdersSubscription) processSSEStream(resp *http.Response) error {
	...
	for scanner.Scan() {
		...
		case "data":
			if event.Event == "ORDER" {
				var orderData OrderEventData
				if err := json.Unmarshal([]byte(value), &orderData); err != nil {
					s.trySendError(fmt.Errorf("parse order data: %w", err))
					continue
				}
